...
class StreamTest extends FreeSpec with Matchers {
  "Stream" - {
    ...
    "fromUnfold" in {
      st.fromUnfold(3).take(5).toList should be(List(3, 4, 5, 6, 7))
    }
    "fib" in {
      st.fibs.take(7).toList should be(List(0, 1, 1, 2, 3, 5, 8))
    }
    "fibs unfold" in {
      st.fibsUnfold.take(7).toList should be(List(0, 1, 1, 2, 3, 5, 8))
    }
    "constantUnfold" in {
      st.constantUnfold(1).take(5).toList should be(List(1, 1, 1, 1, 1))
    }
    "onesUnfold" in {
